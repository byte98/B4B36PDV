...
#pragma omp parallel for
    for(unsigned int i = 0 ; i < params.steps ; i++) {
        ...
        // Doplnte desifrovaci pravidlo:
        //   1) aktualizujte hodnotu string[index] pomoci desifrovaciho pravidla (pouzijte
        //      metodu UNMAP pro prevod z intervalu [0 .. alphabet_size-1] zpet do znaku)
        //   2) aktualizujte index
        string[index] = UNMAP((c3 + (params.p1 * currentSecret)) % alphabet_size);
        index = (index + (params.p2 * currentSecret)) % alphabet_size;
